...
const SingleGridItem = ({ item }: { item: IProduct }) => {
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 h-[250px] mb-4 p-3">
        <Image
        ...
        <div className="absolute left-0 bottom-0 translate-y-[101%] w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          <button
            onClick={() => {
              openModal()
      ...
      <h3 className="font-medium text-dark ease-out duration-200 hover:text-primary mb-1.5">
        <Link href={APP_ROUTES.getDetailProductPath(item._id)}> {item.name} </Link>
      </h3>

      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-dark">${formatPrice(item.sellingPrice)}</span>
        {item.originalPrice && (
          <span className="text-dark-4 line-through">${formatPrice(item.originalPrice)}</span>
        )}
      </span>
    </div>
  )
