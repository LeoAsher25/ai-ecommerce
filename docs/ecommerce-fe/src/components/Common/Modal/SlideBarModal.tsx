// src/components/Common/Modal/SlideBarModal.tsx
...
const SlideBarModal = ({ open, title, children, onClose }: SlideBarModalProps) => {
  useEffect(() => {
    if (open) {
  ...
  return ReactDOM.createPortal(
    <div
      className={clsx('fixed top-0 left-0 z-99999 overflow-y-auto no-scrollbar w-full h-screen', {
        'translate-x-0': open,
    ...
    >
      <div
        className={clsx(
          'fixed top-0 left-0 z-99999 overflow-y-auto no-scrollbar w-full h-screen bg-dark/70 ease-linear duration-300',
      ...
      <div className="flex items-center justify-end h-screen">
        <div className="w-full max-w-[500px] shadow-1 bg-white px-4 sm:px-7.5 lg:px-11 relative modal-content">
          <div className="sticky top-0 bg-white flex items-center justify-between pb-7 pt-4 sm:pt-7.5 lg:pt-11 border-b border-gray-3 mb-7.5">
            <h2 className="font-medium text-dark text-lg sm:text-2xl">{title}</h2>
            <button
              onClick={onClose}
