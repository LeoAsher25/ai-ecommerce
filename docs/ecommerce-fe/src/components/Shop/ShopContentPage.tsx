...
interface IShopContentPageProps {
  productsList: IProduct[]
  totalItems: number
}

const ShopContentPage = ({ productsList, totalItems }: IShopContentPageProps) => {
  ...
  const categorySlug = searchParams.get('phan-loai')
  ...
  const options = [
    ...
    { label: 'Sản phẩm cũ nhất', value: '2' },
  ]
  ...
  const fromIndex = (paginationQuery.page - 1) * paginationQuery.pageSize + 1
  const toIndex =
    paginationQuery.page * paginationQuery.pageSize > totalItems
      ? totalItems
  ...
  return (
    <>
      <Breadcrumb
        title={selectedCategory ? selectedCategory.name : 'Cửa hàng'}
        items={[{ name: 'Cửa hàng', path: APP_ROUTES.SHOP }].concat(
          selectedCategory
            ? [
                {
                  name: selectedCategory.name,
                  path: `${APP_ROUTES.SHOP}?phan-loai=${selectedCategory.slug}`,
                },
              ]
            : []
        )}
      />
      <section className="overflow-hidden relative pb-20 pt-5 lg:pt-6 bg-[#f3f4f6]">
          ...
          <div className="flex gap-7.5">
                ...
                <div className="flex flex-col gap-6">
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    <div className="flex items-center justify-between">
                      <p>Filters:</p>
                      <button className="text-primary">Clean All</button>
                    </div>
                  </div>
            ...
            <div className="w-full">
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <p>{`Hiển thị ${fromIndex}-${toIndex} của ${totalItems} sản phẩm`}</p>
                  </div>
              ...
              <div
                className={`${
                  productStyle === 'grid'
                    ? 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-9'
              ...
              <div className="mt-10">
                <Pagination
                  currentPage={paginationQuery.page ?? DEFAULT_PAGE}
                  pageSize={paginationQuery.pageSize ?? DEFAULT_PAGE_SIZE}
